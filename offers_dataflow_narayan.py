...
def fetch_product_item_mapping():
    ...
    """
    return ReadFromBigQuery(query=query)
...
def run(argv=None):
    ...
    with beam.Pipeline(options=options) as p:
        ...
        product_item_mapping = (
            p | 'FetchProductItemMapping' >> fetch_product_item_mapping()
            | 'FormatProductItemMapping' >> beam.Map(lambda row: (row['ITEM_NBR'], row))
        )
